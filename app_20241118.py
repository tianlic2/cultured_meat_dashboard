...
def load_data():
    ...
    try:
        ...
        df = pd.read_csv(
            ...
            sep='\t',           # 使用制表符作为分隔符
            encoding='utf-8',    # 指定编码
            parse_dates=['Day'], # 直接解析日期列
            date_parser=lambda x: pd.to_datetime(x, format='%m/%d/%y', errors='coerce')  # 指定日期格式
        )
        ...
        st.error("Data columns found: " + ", ".join(df.columns.tolist()) if 'df' in locals() else "No data loaded")
        return pd.DataFrame()  # 返回空DataFrame以防加载失败
