...
def create_word_cloud(df):
    ...
    if generate:
        ...
        with st.spinner('Processing text...\nIt might take up to 1 minute'):
            ...
            # Convert frequencies to percentages of total
            total_freq = sum(top_words.values())
            top_words_pct = {word: (freq/total_freq)*100 for word, freq in top_words.items()}
            
            bars = ax2.bar(range(len(top_words_pct)), list(top_words_pct.values()), 
                         color='steelblue')
            ax2.set_xticks(range(len(top_words_pct)))
            ax2.set_xticklabels(list(top_words_pct.keys()), rotation=45, ha='right')
            ...
            for bar in bars:
                ...
                ax2.text(bar.get_x() + bar.get_width()/2., height,
                        f'{height:.1f}%',  # Display one decimal place
                        ha='center', va='bottom')
            ...
            freq_df = pd.DataFrame({
                'Word': list(word_freq.keys())[:10],
                'Count': list(word_freq.values())[:10],
                'Percentage': [f"{(count/total_freq)*100:.1f}%" for count in list(word_freq.values())[:10]]
            })
